...
import { PUBLISHED_VARIANT } from '@nrz/infra-build'
...
const fixLicense = async (ws: Workspace) => {
  ...
  const containsNrz = () => {
    if (!license.includes('Copyright (c) KhulnaSoft, Ltd.')) {
      throw new Error(
        `${ws.pj.name} license should contain nrz company name`,
      )
    }
  }
...
const fixCliVariants = async (ws: Workspace) => {
  ...
  ws.pj.description = `The nrz CLI${descriptionExtra}`
  await writeFormatted(
    resolve(ws.dir, 'README.md'),
    readFileSync(resolve(ws.dir, 'README.md'), 'utf8')
      .replaceAll(/`nrz`( \(.*\))?/g, `\`nrz\`${descriptionExtra}`)
