...
import type { PostcssNode as _PostcssNode } from '@nrz/dss-parser'
import { joinDepIDTuple, splitDepID } from '@nrz/dep-id/browser'
...
t.test(':path selector', async t => {
  ...
  await t.test('excludes .nrz store paths', async t => {
    // This test verifies that even if a .nrz path pattern is used,
    // only workspace and file nodes are matched (not registry nodes)
    const res = await path(getState(':path("**/.nrz/**")'))
    t.strictSame(
      [...res.partial.nodes].map(n => n.name),
      [],
      'should not match any nodes for .nrz store patterns since only workspace/file nodes are considered',
    )
  })
