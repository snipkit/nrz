...
import { asDepID, splitDepID, joinDepIDTuple } from '@nrz/dep-id'
...
import type { GraphLike } from '@nrz/graph'
  ...
  implements SecurityArchiveLike
{
  ...
  constructor(options: SecurityArchiveOptions = {}) {
    ...
    this.#path =
      options.path ?? new XDG('nrz').cache('security-archive.db')
  ...
  #loadItemsFromDatabase(db: DatabaseSync, graph: GraphLike): void {
    ...
    // cli commands that make usage of the security-archive, e.g: nrz ls,
    // nrz query to not hang while waiting for stale-while-revalidate
  ...
  async #retrieveRemoteData(
    ...
    const req = await fetch(SOCKET_API_V0_URL, {
      ...
      headers: {
        ...
        'User-Agent': `@nrz/security-archive/${version}`,
      },
      body: JSON.stringify({
