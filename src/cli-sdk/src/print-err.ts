...
import type { Node } from '@nrz/graph'
import type { ParsedError } from '@nrz/output/error'
import { parseError } from '@nrz/output/error'
import { isErrorWithCause, isObject } from '@nrz/types'
...
const writeErrorLog = (e: unknown, format: Formatter) => {
  try {
    const dir = new XDG('nrz/error-logs').data()
...
export const printErr = (
  ...
  if (!knownError || knownError.bug) {
    ...
    stderr(indent('https://github.com/khulnasoft-lab/nrz/issues/new'))
  }
}
