...
const usageDef = {
  command: 'cache',
  usage: '<command> [flags]',
  description: 'Work with nrz cache folders',
  ...
  subcommands: {
    ...
    'delete-all': {
      usage: '',
      description: `Delete the entire cache folder to make nrz slower.`,
    },
  },
  examples: {
    'nrz cache ls https://registry.npmjs.org/typescript': {
      description: `Show cache metadata for a given registry URL`,
    },
    'nrz cache add eslint@latest': {
      description: `Add a given package specifier to the cache by fetching
                    its resolved value.`,
    },
    'nrz cache info https://registry.npmjs.org/eslint/-/eslint-9.25.1.tgz > eslint.tgz':
      {
        description: `Print the cache metadata to stderr, and write the tarball
                    on stdout, redirecting to a file.`,
      },
    'nrz cache delete-before 2025-01-01': {
