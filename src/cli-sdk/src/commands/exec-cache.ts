...
import type { VlxInfo } from '@nrz/nrx'
...
const usageDef = {
  command: 'exec-cache',
  usage: '<command> [flags]',
  description: 'Work with nrz exec-cache folders',

  subcommands: {
    ls: {
      usage: '',
      description: `Show previously installed packages used for \`nrz exec\`.
    ...
    delete: {
      ...
      description: `Delete previously installed packages used for
                    \`nrz exec\`. If no keys are provided, then all entries
                    will be deleted.`,
    },

    info: {
      usage: '<key>',
      description: `Show extended information about a given \`nrz exec\`
                    installation.`,
    },

    install: {
      usage: '<spec>...',
      description: `Install the specified package(s) in the \`nrz exec\`
  ...
  examples: {
    ls: {
      description: `Show all the keys for the installations in the \`nrz exec\`
                    cache.`,
    },
    'delete typescript': {
      description: `Delete all versions of typescript installed for nrz exec`,
    },
...
const install = async (
  ...
  return Promise.all(
    keys.map(async key => {
      const info = await nrx.install(key, {
        ...conf.options,
        query: undefined,
      })
      view?.stdout(info)
...
const ls = async (
  ...
  for await (const path of nrx.list()) {
...
const info = async (
  ...
  for await (const key of keys.length ? keys : nrx.list()) {
    const info = nrx.info(key, {
      ...conf.options,
      query: undefined,
    })
    results.push(info)
