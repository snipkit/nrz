/**
 * impl for `nrz run`, `nrz run-exec`, `nrz exec-local`, `nrz exec`
 * @module
 */

import { error } from '@nrz/error-cause'
import { isErrorWithCause } from '@nrz/types'
import type {
  exec,
  execFG,
  ExecOptions,
...
import { isRunResult } from '@nrz/run'
...
export class ExecCommand<B extends RunnerBG, F extends RunnerFG> {
  ...
  // overridden by 'nrz run' which returns undefined
