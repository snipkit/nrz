...
t.test('read and write a project config', async t => {
  ...
  const dir = t.testdir({
    'nrz.json':
      JSON.stringify(
        { config: { registry: 'https://registry.nrz.sh/' } },
        null,
        '\t',
      ) + '\n',
    a: { b: {} },
  ...
  t.equal(conf.get('registry'), 'https://registry.nrz.sh/')
  ...
  t.strictSame(
    JSON.parse(readFileSync(dir + '/nrz.json', 'utf8')),
    {
      config: {
        cache: 'hello',
        registry: 'https://registry.nrz.sh/',
      },
    },
...
t.test('add to empty config adds a config file', async t => {
  ...
  t.strictSame(JSON.parse(readFileSync(dir + '/nrz.json', 'utf8')), {
    config: { cache: 'cache' },
  })
})
...
t.test('invalid config', async t => {
  ...
  t.test('invalid object', async t => {
    ...
    >('../../src/config/index.ts')
    const dir = t.testdir({
      'nrz.json': JSON.stringify('this is not valid json'),
      '.git': '',
    })
