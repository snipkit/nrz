...
If \`--package\` is not set, then nrz will attempt to infer the package to be
...
- Otherwise, nrz attempts to resolve the first argument as if it was a
...
At no point will \`nrz exec\` change the locally installed dependencies. Any
installs it performs is done in nrz's XDG data directory.
  ...
  Examples
    ...
    ​nrz exec --package typescript@5 tsc

    Run the default bin provided by eslint

    ​nrz exec eslint src/file.js

    Run the default bin provided by eslint version 9.24

    ​nrz exec eslint@9.24 src/file.js
