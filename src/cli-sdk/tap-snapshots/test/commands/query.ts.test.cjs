...
Usage:
  ...
  nrz query <query> --expect-results=<comparison string>

List installed dependencies matching the provided query.

The nrz Dependency Selector Syntax is a CSS-like query language that allows you
...
CSS-like attributes, pseudo selectors & combinators.
  ...
  Examples
    ...
    ​nrz query '*:workspace > *:peer'

    Query all direct project dependencies with a "build" script

    ​nrz query ':project > *:attr(scripts, [build])'
    ...
    ​nrz query '*:license(copyleft) --expect-results=0'
