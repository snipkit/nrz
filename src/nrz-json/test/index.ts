...
t.test('load, edit, save existing project file', async t => {
  ...
  t.equal(find(), resolve(dir, 'nrz.json'))
  let validatorRan = false
  const foo = load('foo', (x): x is { foo: boolean } => {
    validatorRan = true
  ...
  save('bar', true)
  t.equal(
    readFileSync(resolve(dir, 'nrz.json'), 'utf8'),
    JSON.stringify(
      {
        foo: {
          foo: true,
...
t.test('do not clobber file if edited out of band', async t => {
  ...
  t.equal(foo, undefined)
  t.equal(validatorRan, false)
  writeFileSync(resolve(dir, 'a/b/nrz.json'), '{}')
  ...
  save('foo', { foo: true })
  t.equal(
    readFileSync(resolve(dir, 'a/b/nrz.json'), 'utf8'),
...
t.test('do not use user config as project config', async t => {
  class MockXDG extends XDG {
    config() {
      return resolve(t.testdirName, 'nrz.json')
    }
  }
  ...
  const { load, save, find } = await t.mockImport<
    typeof import('../src/index.ts')
  >('../src/index.ts', { '@nrz/xdg': { XDG: MockXDG } })
  t.equal(find(), resolve(dir, 'a/b/nrz.json'))

  function assertBoolean(b: unknown): asserts b is boolean {
    if (b !== !!b) throw new Error('nope')
  }

  t.equal(find('user'), resolve(dir, 'nrz.json'))
...
t.test('missing nrz.json is no big deal', async t => {
  class MockXDG extends XDG {
    config() {
      return resolve(t.testdirName, 'nrz.json')
    }
...
t.test('fail to parse nrz.json file', async t => {
  class MockXDG extends XDG {
    config() {
      return resolve(t.testdirName, 'nrz.json')
    }
  }
  const dir = t.testdir({ 'nrz.json': 'hello' })
  const { load } = await t.mockImport<
    typeof import('../src/index.ts')
  >('../src/index.ts', {
    '@nrz/xdg': { XDG: MockXDG },
...
t.test('do not walk up past a .git folder', async t => {
  const dir = t.testdir({
    'nrz.json': JSON.stringify({ root: true }),
    a: {
      '.git': {},
      b: {
        'package.json': JSON.stringify({}),
  ...
  t.equal(find('project'), resolve(dir, 'a/b/nrz.json'))
