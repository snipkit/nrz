...
t.test('delete everything', async t => {
  ...
  const { nrxDelete } = await t.mockImport<
    typeof import('../src/delete.ts')
  >('../src/delete.ts', {
    '../src/list.ts': { nrxList: mockVlxList(dir) },
    '../src/info.ts': mockVlxInfo,
  })
  const results = await nrxDelete([], remover, options)
  t.match(
    new Set(results),
...
t.test('delete by key', async t => {
  ...
  const results = await nrxDelete(['hashnobin'], remover, options)
...
t.test('missing key, still deletes bad entry', async t => {
  ...
  const results = await nrxDelete([k], remover, options)
  t.match(results, [])
  t.strictSame(
