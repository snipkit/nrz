...
import { reload } from '@nrz/nrz-json'
...
const newNode =
  ...
  (name: string, version = '1.0.0'): NodeLike => ({
    ...
    name,
    version,
    location: `node_modules/.nrz/joinDepIDTuple('registry', '', name + '@' + version)/node_modules/${name}`,
...
t.test('GraphModifier', async t => {
  await t.test('constructor', async t => {
    ...
    const emptyconfigDir = t.testdir({
      'nrz.json': JSON.stringify({}), // No modifiers key
    })
    const options = {
      ...mockSpecOptions,
    }
    // Reload nrz.json to ensure we have the latest config
  ...
  await t.test('config getter', async t => {
    const testdir = t.testdir({
      'nrz.json': JSON.stringify({ modifiers: validStringConfig }),
    })
    ...
    // Test with config from nrz.json
    t.same(
      modifier.config,
      validStringConfig,
      'should load config from nrz.json',
    )
