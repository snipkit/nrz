...
t.test('basic hoisting', async t => {
    ...
    node_modules: {
      ...
      '@nrz': {
        foo: t.fixture(
          ...
          `../.nrz/${joinDepIDTuple([
            'git',
            'git@github.com/khulnasoft-lab/foo',
            'main',
          ])}/node_modules/@nrz/foo`,
        ),
      },
      // outside of nrz store, not hoisted
    ...
    src: {
      foo: {
        ...
        node_modules: {
          abbrev: t.fixture(
            'symlink',
            `../../../node_modules/.nrz/${joinDepIDTuple(['registry', '', 'abbrev@1.1.1'])}/node_modules/abbrev`,
          ),
        },
      },
    },
  ...
  t.strictSame(
    readdirSync(
      resolve(projectRoot, 'node_modules/.nrz/node_modules'),
    ).sort((a, b) => a.localeCompare(b, 'en')),
    ['@nrz', 'abbrev'],
  )
  t.strictSame(
    readdirSync(
      resolve(projectRoot, 'node_modules/.nrz/node_modules/@nrz'),
    ),
    ['foo'],
  )
})
...
t.test('deciding which node to hoist', async t => {
  ...
  t.test('do not pick node not in nrz store', async t => {
    const n = {
      importer: false,
      inNrzStore: () => false,
    } as unknown as Node
