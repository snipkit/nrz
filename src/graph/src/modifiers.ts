import { parseBreadcrumb, specificitySort } from '@nrz/dss-breadcrumb'
import { error } from '@nrz/error-cause'
import { Spec } from '@nrz/spec'
import { asManifest, assertRecordStringString } from '@nrz/types'
import { load } from '@nrz/nrz-json'
import type {
  ModifierBreadcrumb,
  ModifierInteractiveBreadcrumb,
} from '@nrz/dss-breadcrumb'
...
/**
 * Loaded modifiers configuration as described in the `nrz.json` file.
 */
...
export class GraphModifier {
  ...
  /** A set of all modifiers loaded from nrz.json */
  #modifiers = new Set<ModifierEntry>()
  /** A set of all edge modifiers loaded from nrz.json */
  #edgeModifiers = new Set<EdgeModifierEntry>()
  /** A set of all node modifiers loaded from nrz.json */
  ...
  /**
   * Load the modifiers definitions from nrz.json,
  ...
  /**
   * Loads the modifiers defined in `nrz.json` into memory.
   */
   ...
   * Returns undefined if the project does not have a nrz.json file,
  ...
  /**
   * Convenience method to instantiate and load in one call.
   * Throws if called on a directory that does not have a nrz.json file.
   */
