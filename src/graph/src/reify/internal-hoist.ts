...
import { Version } from '@nrz/semver'
...
export const pickNodeToHoist = (
  ...
  for (const node of nodes) {
    if (node.importer || !node.inNrzStore()) {
      continue
    }
...
export const internalHoist = async (
  ...
  for (const [name, id] of links) {
    const target = scurry.resolve(
      `node_modules/.nrz/${id}/node_modules/${name}`,
    )
    const path = scurry.resolve(
      `node_modules/.nrz/node_modules/${name}`,
    )
    if (name.includes('/')) {
