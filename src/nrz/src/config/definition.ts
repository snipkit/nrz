...
const j = jack({
  ...
  .heading('nrz')
  .description(
    `More documentation available at <https://docs.khulnasoft.com>`,
  )
  .heading('Subcommands')
...
export const definition = j
  ...
  .opt({
    ...
    'script-shell': {
      ...
      description: `The shell to use when executing \`package.json#scripts\`
                    ...
                    \`nrz run\`) and \`nrz exec\`.

                    If not set, defaults to \`/bin/sh\` on POSIX systems,
                    and \`cmd.exe\` on Windows.

                    When no argument is provided to \`nrz exec\`, the \`SHELL\`
  ...
  .opt({
    package: {
      hint: 'p',
      description: `When running \`nrz install-exec\`, this allows you to
                    explicitly set the package to search for bins. If not
                    provided, then nrz will interpret the first argument as
