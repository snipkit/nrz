/**
 * Module that handles all nrz configuration needs
 *
 * Project-level configs are set in a `nrz.json` file in the local project
...
/**
 * Class that handles configuration for nrz.
 *
...
export class Config {
   ...
   * representing nrz's configuration
   */
  ...
  /**
   * positional arguments to the nrz process
   */
  positionals?: string[]

  /**
   * The root of the project where a nrz.json, nrz-workspaces.json,
  ...
  async #readConfigFile(
    ...
    try {
      ...
      throw error('failed to parse nrz config file', {
        path: file,
        cause: er,
      })
    }
  ...
  getFilename(which: 'project' | 'user' = 'project'): string {
    return which === 'user' ?
        xdg.config('nrz.json')
      : resolve(this.projectRoot, 'nrz.json')
  }
   ...
   * the XDG config home.
   *
   * Note: if working in a workspaces monorepo, then the nrz.json file MUST
   * be in the same folder as the nrz-workspaces.json file, because we stop
   * looking when we find either one.
   */
  async loadConfigFile(): Promise<this> {
    const userConfig = xdg.config('nrz.json')
    ...
    for (const dir of walkUp(this.projectRoot)) {
      ...
      const projectConfig = resolve(dir, 'nrz.json')
      ...
      const [hasPackage, hasModules, hasWorkspaces, hasGit] =
        await Promise.all([
          ...
          exists(resolve(dir, 'nrz-workspaces.json')),
