/**
 * Implementation shared between `nrz run`, `nrz run-exec`, and `nrz exec`
 */

import { error, isErrorRoot } from '@nrz/error-cause'
import {
  type exec,
  type execFG,
  type ExecOptions,
...
import { Monorepo, type Workspace } from '@nrz/workspaces'
import { ansiToAnsi } from 'ansi-to-pre'
import chalk from 'chalk'
import { type LoadedConfig } from './config/index.ts'
import { stdout, stderr } from './output.ts'
import { type SpawnResultNoStdio } from '@nrz/promise-spawn'
...
export class ExecCommand<B extends RunnerBG, F extends RunnerFG> {
  ...
  // overridden by 'nrz run' to print options and return undefined
  ...
  // overridden in `nrz run` to print available scripts
  noArgsMulti() {
    throw error(
      'Cannot spawn interactive shells in multiple workspaces',
    )
