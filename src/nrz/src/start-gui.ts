...
const createStaticHandler = ({
  ...
  if (process.env._NRZ_DEV_LIVE_RELOAD) {
...
const getAssetsDir = () => {
  ...
  if (process.env.TAP) {
    assert(
      process.env.NRZ_TEST_GUI_DIR,
      'NRZ_TEST_GUI_DIR must be set when running tests',
    )
    return process.env.NRZ_TEST_GUI_DIR
  }
  return fileURLToPath(import.meta.resolve('@nrz/gui'))
}
/* c8 ignore stop */
...
export const startGUI = async ({
  ...
  const tmp = resolve(tmpDir, 'nrzgui')
  ...
  return new Promise<Server>(res => {
    ...
    server.listen(port, 'localhost', () => {
      stdout(`⚡️ nrz GUI running at http://${HOST}:${port}`)
