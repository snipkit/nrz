...
Object {
  ...
  "package": Object {
    "description": "When running \`nrz install-exec\`, this allows you to explicitly set the package to search for bins. If not provided, then nrz will interpret the first argument as the package, and attempt to run the default executable.",
    "hint": "p",
    "type": "string",
  },
  "recursive": Object {
  ...
  "script-shell": Object {
    "description": String(
      The shell to use when executing \`package.json#scripts\` (either as lifecycle scripts or explicitly with \`nrz run\`) and \`nrz exec\`.
      
      If not set, defaults to \`/bin/sh\` on POSIX systems, and \`cmd.exe\` on Windows.
      
      When no argument is provided to \`nrz exec\`, the \`SHELL\` environment variable takes precedence if set.
    ),
    "hint": "program",
    "type": "string",
  },
