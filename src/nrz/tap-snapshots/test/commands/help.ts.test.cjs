...
More documentation available at <https://docs.khulnasoft.com>
  ...
  Configuration
    ...
    If a \`nrz.json\` file is present in the root of the current project, then
    that will be used as a source of configuration information.

    Next, the \`nrz.json\` file in the XDG specified config directory will be
  ...
  --registries=<name=url>
                       ...
                       \`\`\`
                       ​nrz --registries loc=http://reg.local install
                       foo@loc:foo@1.x
                       \`\`\`
  ...
  --scope-registries=<@scope=url>
                       ...
                       @acme=https://registry.acme/\` would tell nrz to fetch any
  ...
  --cache=<path>       Location of the nrz on-disk cache. Defaults to the
  ...
  -i<name> --identity=<name>
                       ...
                       used, storing the file at \`nrz/auth/keychain.json\`.
  ...
  --editor=<program>   The blocking editor to use for \`nrz config edit\` and any
  ...
  --script-shell=<program>
                       ...
                       (either as lifecycle scripts or explicitly with \`nrz
                       run\`) and \`nrz exec\`.
  ...
  --package=<p>        When running \`nrz install-exec\`, this allows you to
