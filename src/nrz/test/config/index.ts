...
t.test('read and write a project config', async t => {
  const dir = t.testdir({
    'nrz.json':
      JSON.stringify(
        { registry: 'https://registry.khulnasoft.com/' },
        null,
        '\t',
      ) + '\n',
    a: { b: {} },
  ...
  t.equal(conf.get('registry'), 'https://registry.khulnasoft.com/')
  ...
  t.match(
    readFileSync(dir + '/nrz.json', 'utf8'),
    /^\{\n\t"/,
    'preserved indentation',
  )
  unlinkSync(dir + '/nrz.json')
  await conf.addConfigToFile('project', { cache: 'hello' })
  t.strictSame(
    JSON.parse(readFileSync(dir + '/nrz.json', 'utf8')),
    {
      cache: 'hello',
      registry: 'https://registry.khulnasoft.com/',
    },
...
t.test('read and write a user config', async t => {
  ...
  t.equal(c2.get('cache'), dir + '/cache')
  t.match(
    readFileSync(dir + '/xdg/nrz.json', 'utf8'),
    /^\{\n\t"/,
    'preserved indentation',
  )
  unlinkSync(dir + '/xdg/nrz.json')
  await conf.addConfigToFile('user', { cache: 'hello' })
  t.strictSame(
    JSON.parse(readFileSync(dir + '/xdg/nrz.json', 'utf8')),
    {
      registry: 'https://registry.khulnasoft.com/',
      cache: 'hello',
    },
...
t.test('enable/disable color output', async t => {
  ...
  const mockXDG = {
    XDG: class XDG {
      config() {
        return resolve(dir, 'xdg/nrz.json')
      }
      cache() {
  ...
  t.test('enable color if config says to', async t => {
    writeFileSync(dir + '/nrz.json', JSON.stringify({ color: true }))
  ...
  t.test('disable colors when config says to', async t => {
    writeFileSync(dir + '/nrz.json', JSON.stringify({ color: false }))
    const env: Record<string, string> = { NO_COLOR: '0' }
    t.intercept(process, 'env', { value: env })
    const chalk = { default: { level: 3 } }
    const { Config } = await t.mockImport(
    ...
    t.test('enable colors when cli says to', async t => {
      ...
      rmSync(dir + '/nrz.json', { force: true })
      const env: Record<string, string> = { NO_COLOR: '0' }
      t.intercept(process, 'env', { value: env })
      const chalk = { default: { level: 0 } }
      const { Config } = await t.mockImport(
...
t.test('invalid config', async t => {
  t.test('invalid json', async t => {
    const dir = t.testdir({
      'nrz.json': 'this is not valid json',
      '.git': '',
    })
    await t.rejects(Config.load(dir, undefined, true), {
      cause: {
        path: resolve(dir, 'nrz.json'),
        cause: {
          name: 'JSONParseError',
          code: 'EJSONPARSE',
        },
  ...
  t.test('valid json, invalid data', async t => {
    const dir = t.testdir({
      'nrz.json': JSON.stringify({ color: 'not a boolean' }),
      '.git': '',
    })
    await t.rejects(Config.load(dir, undefined, true), {
      cause: {
