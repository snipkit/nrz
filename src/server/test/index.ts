...
const { createServer, NrzServer } = await t.mockImport<
  typeof import('../src/index.ts')
>('../src/index.ts', MOCKS)

t.type(
  createServer({} as unknown as NrzServerOptions),
  NrzServer,
...
t.test('start listening', async t => {
  ...
  s1.listening = function (): this is NrzServerListening {
    return true
  }
  await t.rejects(s1.close())
