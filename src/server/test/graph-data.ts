...
t.test('graph data for nrz project', async t => {
  const { updateGraphData } = await t.mockImport<
    typeof import('../src/graph-data.ts')
  >('../src/graph-data.ts', {
    '@nrz/security-archive': {
  ...
  const dir = t.testdir({
    ...
    project: {
      ...
      node_modules: {
        abbrev: t.fixture('symlink', `.nrz/${abbrevDepID}`),
        '.nrz': {
          [abbrevDepID]: {
            'package.json': JSON.stringify({
              name: 'abbrev',
  ...
  t.strictSame(result, {
    ...
    projectInfo: { tools: ['nrz'], nrzInstalled: true },
    securityArchive: {
      [abbrevDepID]: {
        id: '99923218962',
        author: ['npm'],
...
t.test('graph data for depless nrz project', async t => {
  const dir = t.testdir({
    tmp: {
      'graph.json': JSON.stringify({
        just: { some: { prexisting: 'junk' } },
