...
The nrz client supports working with multiple registries in a variety
of ways.
...
[VSR](https://www.nrz.sh/serverless-registry) is a hosted serverless
registry that nrz provides for this purpose.
...
By default, nrz fetches packages from
...
```bash
nrz config set registry=https://registry.yarnpkg.com
```
...
Like the npm client, the nrz client allows you to map a given scope to
a specific registry.
...
To do this in nrz, use the `scope-registries` configuration.

```bash
nrz config set scope-registries=@mycompany=https://registry.local/
```

When you do this in your project, you will see that it adds the option
to a `nrz.json` file in your project root.
...
version, the nrz client allows you to indicate that you want a given
...
```bash
$ nrz config set registries=bar=https://registry.example.com/
```

This will add the appropriate entry to the `nrz.json` file:
...
```bash
$ nrz install jsr:@am/neuralNetwork@1.0.0
```
...
```bash
$ nrz install gh:@octocat/hello-world@1.0.0
```
...
of multiple registries, which nrz handles properly.
...
So the concern is, if I do `nrz install foo:foo@1.2.3`, then it'll
...
The nrz client handles this by defaulting every dependency to the same
...
At the time of this writing, only the nrz client supports arbitrary
...
```bash
$ nrz install "foo@registry:https://foo.com#foo@1"
```
...
code, because currently only the nrz client has support for this type
of specifier.
