...
<Code
  code="$ nrz [<options>] [<cmd> [<args> ...]]"
  title="Terminal"
  lang="bash"
/>

More documentation available at
[https://docs.nrz.sh](https://docs.nrz.sh)
...
workspaces and other `nrz` behaviors.)

Next, the `nrz.json` file in the XDG specified config directory will
...
<Code
  code="​​$ nrz --registries loc=http://reg.local install foo@loc:foo@1.x"
  title="Terminal"
  lang="bash"
/>
...
tell nrz to fetch any packages named `@acme/...` from the
...
`nrz/auth/${identity}/keychain.json`.

If no identity is provided, then the default `''` will be used,
storing the file at `nrz/auth/keychain.json`.
...
The blocking editor to use for `nrz config edit` and any other cases
...
lifecycle scripts or explicitly with `nrz run`) and `nrz exec`.

If not set, defaults to `/bin/sh` on POSIX systems, and `cmd.exe` on
Windows.

When no argument is provided to `nrz exec`, the `SHELL` environment
...
<Code
  code="​​$ ​​nrz config set fallback-command=run-exec"
  title="Terminal"
  lang="bash"
/>
...
When running `nrz exec`, this allows you to explicitly set the package
to search for bins. If not provided, then nrz will interpret the first
