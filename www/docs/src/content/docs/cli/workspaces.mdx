---
title: Using Workspaces in a Monorepo Project with nrz
sidebar:
  label: Workspaces
  order: 4
---
...
## Using Workspaces in a Monorepo Project with nrz

When using the `nrz` client, you can define multiple different
workspaces in a single monorepo project.

The `nrz` client supports workspaces defined by glob patterns, and you
can also define named groups to perform nrz operations on a specific
subset of your workspaces.

## Defining Workspaces

To use monorepo features, create a `nrz.json` file in the root of your
...
`cd` into that folder, and then use whatever nrz operation as you
...
```bash
$ nrz --workspace=src/* --workspace=www/components run test
```
...
For example, with the `nrz.json` defined above:
