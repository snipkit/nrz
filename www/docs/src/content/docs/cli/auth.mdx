...
The nrz client supports logging into multiple different registries.

## npm Web Login

To log into a registry that supports the npm web login, such as the
public npm registry, use the `nrz login` command.
...
<Code
  code="$ nrz login --registry=https://my-custom-registry.mycompany.local/"
  title="Terminal"
  lang="bash"
/>

To remove your authorization to a given registry, including attempting
to destroy the token on the server side, use the `nrz logout` command.

## Other Registries

If your registry provides a bearer token that should be presented for
authentication, it can be added using the `nrz token add` command.

This will prompt you to paste in the token.

To remove a token from nrz's keychain, use the `nrz token rm` command.
...
A token for the default registry can be provided in the `NRZ_TOKEN`
...
The nrz client is designed to make it less likely that passwords are
...
With nrz, sensitive information is _only_ kept in a tightly restricted
...
Each "identity" in nrz is a set of authentication tokens for known
registries.
...
Running `nrz config set identity=corp` will switch into `corp` mode.
`nrz config set identity=personal` will switch into `personal` mode.

This can also be set in a project root with a `nrz.json` config file,
to make it easier to set per project. For example, you could have this
in your `nrz.json` in the project root:
